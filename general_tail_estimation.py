...
def plot_countour(df, thresholds, season, grid_name, save_path):
	# Extract example values from the DataFrame for plotting
	...
	for threshold in thresholds:
		if direction == 'right':  # from threshold to right [x_th, inf)
			...
			tail_data = df[df['Value'] <= threshold]['Value']
			x_data = (50.00 - tail_data).sort_values(ascending=False).values
			x_k = threshold
		else:
		...
		plt.figure(figsize=(10, 6))
		cp = plt.contourf(A, B, LL, levels=50, cmap='viridis')
		plt.colorbar(cp, label='Log-Likelihood')
		plt.xlabel('Alpha')
		plt.ylabel('Beta')
		plt.title(f'Log-Likelihood Surface ({season} - {grid_name})')
		plt.grid(True)
		plt.savefig(os.path.join(save_path, f"x_th_{threshold}_countour.png"))
		plt.show()

def plot_likelihood_fixed_beta(df, thresholds, beta, season, grid_name, save_path):
	# Log-likelihood function
	def _log_likelihood(alpha, beta, data, x_k):
		k = len(data)
		term1 = k * (np.log(alpha) + np.log(beta) + alpha * x_k ** beta)
	...
	for threshold in thresholds:
		if direction == 'right':  # from threshold to right [x_th, inf)
			...
			tail_data = df[df['Value'] <= threshold]['Value']
			x_data = 50 - (50.00 - tail_data).sort_values(ascending=False).values
			x_k = threshold
		else:
			raise ValueError("Direction must be 'right' or 'left'.")
		...
		plt.title(f'[base beta={beta}, x_th={threshold}] Log-Likelihood vs Alpha ({season} - {grid_name})')
		plt.xlabel('Alpha')
		plt.ylim(0, 1e6)
...
def plot_likelihood_fixed_alpha(df, thresholds, alpha, season, grid_name, save_path):
	# Log-likelihood function
	def _log_likelihood(alpha, beta, data, x_k):
		k = len(data)
		term1 = k * (np.log(alpha) + np.log(beta) + alpha * x_k ** beta)
	...
	for threshold in thresholds:
		...
		plt.title(f'[base alpha={alpha}, x_th={threshold}] Log-Likelihood vs Beta ({season} - {grid_name})')
		plt.xlabel('Beta')
		plt.xlim(1, 2.5)
		plt.ylim(0, 1e6)
		plt.ylabel('Log-Likelihood')
...
def optimize_thresholds_heatmap(df, thresholds, alpha_range, direction, grid_name, save_path):
	...
	if results:
		...
		ax = sns.heatmap(pivot_ll, annot=True, fmt=".3e", cmap="viridis", cbar_kws={'label': 'Log-Likelihood'})
		...
		# Draw boxes around column-wise minima
		...
		plt.figure(figsize=(12, 6))
		ax = sns.heatmap(pivot_beta, annot=True, fmt=".3f", cmap="magma", cbar_kws={'label': 'Estimated Beta'})
...
for season in seasons[:1]:
	for grid_name in grid_names:
		for direction in directions[:1]:
			save_path = os.path.join(os.getcwd(), "results_new_fix3", season, grid_name, direction)
			...
			if direction == 'right':
				if grid_name == 'european':
					thresholds = np.linspace(50.05, 50.15, 1)
				else:
					thresholds = np.linspace(50.10, 50.20, 1)
			elif direction == 'left':
				if grid_name == 'european':
					thresholds = np.linspace(49.85, 49.95, 10)
				else:
					thresholds = np.linspace(49.80, 49.90, 10)
			else:
				raise ValueError("Direction must be 'right' or 'left'.")

			# plot_countour(df, thresholds, season, grid_name, save_path)
            plot_likelihood_fixed_beta(df, thresholds, beta=1.5, season=season, grid_name=grid_name, save_path=save_path)
            plot_likelihood_fixed_beta(df, thresholds, beta=2.5, season=season, grid_name=grid_name, save_path=save_path)
            # plot_likelihood_fixed_alpha(df, thresholds, alpha=0.35, season=season, grid_name=grid_name, save_path=save_path)
			# plot_likelihood_fixed_alpha(df, thresholds, alpha=1.35, season=season, grid_name=grid_name, save_path=save_path)
